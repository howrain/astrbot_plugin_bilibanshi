...
Cron 使用 AstrBot Core 的统一调度器，按 AstrBot 的有效时区执行；格式为“分 时 日 月 周”，支持数字、`*`、逗号列表、范围和步长，例如 `0 */2 * * *` 表示每两小时整点执行。命中 23:00–次日 07:00 的免打扰时段时会跳过。把免打扰开始和结束时间设为相同值可关闭宵禁。使用 `/bilibanshi cron` 修改后立即重排；在插件设置页修改 Cron 后需重载插件。
